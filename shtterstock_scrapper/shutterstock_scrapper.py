...
def scrape_images_with_selenium(url, num_images):
    options = Options()
...
def main():
    ...
    topic = st.text_input("Enter a topic to download images for (e.g. 'Travel', 'Flower', 'Animal'):")
    image_type = st.selectbox("Image Type", ["photo", "vector", "illustration", "3d-object", "generated"])
    ...
    if st.button("Scrape Images"):
        try:
            ...
            if num_images <= 0:
                ...
                url = f"{base_url}/search/{topic}?image_type={image_type}"
                if library == "Beautiful Soup":
                    ...
                    images = scrape_images_with_selenium(url, num_images)
                    st.write(f"Found {len(images)} images:")
                    st.write(images)
                    download_images(images, output_dir)
                    st.write(f"Images downloaded to {output_dir}")
